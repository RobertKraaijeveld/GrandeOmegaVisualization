...
float GenericVector::dotProduct(GenericVector& other)
{
    //check behavior on sparse data!!!
    float returnValue = 0.0f;

    for(int i = 0; i < values.size(); i++)
    {
        if(i <= other.values.size())
            returnValue += values[i] * other.values[i];            
    }
    return returnValue;
}
...
float GenericVector::getEuclidDistance(GenericVector& other)
{
    if (!this->VectorSizesEqual(other))
    {
        ...
        cout << "!this->VectorSizesEqual(other)" << endl;
        return -1.0;        
    }
...
bool GenericVector::isNotEqual(GenericVector& other)
{
    for(int i = 0; i < values.size(); i++)
    {
